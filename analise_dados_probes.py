#a função do script é analisar os dados das "probes" e gerar gráficos sobre latência, saltos e média geral por país, continente, e site (Kick, YouTube, Twitch).
...
def plot_probes_latency_by_site_individual(df, output_dir):
    ...
    for service in services:
        ...
        plt.gca().xaxis.set_major_formatter(plt.matplotlib.dates.DateFormatter('%d/%m %H:%M'))  # Ajustar o formato da data
        plt.xlabel('Tempo')
        plt.ylabel('Latência (ms)')
        plt.title(f'Latência (RTT) por País - {service}', fontsize=16)
...
def plot_latency_hops_by_country_individual(df, output_dir):
    ...
    for service in services:
        ...
        plt.xlabel('Tempo')
        plt.ylabel('Latência (ms)')
        plt.title(f'Latência (RTT) por País para {service}', fontsize=16)
        ...
        plt.gca().xaxis.set_major_formatter(plt.matplotlib.dates.DateFormatter('%d/%m %H:%M'))  # Ajustar o formato da data
        plt.xlabel('Tempo')
        plt.ylabel('Quantidade de Hops')
        plt.title(f'Quantidade de Hops por País para {service}', fontsize=16)
        plt.legend(title="País", bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
...
def plot_latency_hops_by_continent_individual(df, output_dir):
    ...
    for service in services:
        ...
        plt.xlabel('Tempo')
        plt.ylabel('Latência (ms)')
        plt.title(f'Latência (RTT) por Continente para {service}', fontsize=16)
        ...
        plt.xlabel('Tempo')
        plt.ylabel('Quantidade de Hops')
        plt.title(f'Quantidade de Hops por Continente para {service}', fontsize=16)
        plt.legend(title="Continente", bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
...
def plot_avg_latency_hops_by_site(df, output_dir):
    ...
    plt.xticks(rotation=45)
    plt.xlabel('Site')
    plt.ylabel('Quantidade Média de Hops')
